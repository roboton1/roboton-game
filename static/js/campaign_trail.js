...
e.SelAnsContText = "進む前に選択肢を答えてください！";
...
function divideElectoralVotesProp(e, t) {
    ...
    function election_HTML(id, cand, running_mate) {
        if (id != 16) {
            if (modded) {
                ...
                const yearbit = getScenarioYear();
                console.log("SCENARIO YEAR:", yearbit);

                if (!yearbit) {
                    throw new Error("Scenario year could not be resolved.");
                }

                return baseScenarioDict[String(yearbit)];
            } else {
    ...
    function setStatePollText(s, t) {
        ...
        if (!doPrimaryMode && !e.primary) {
            var c = "<h3>予想マップ</h3>                    <ul id='switchingEst'>" + _ + "</ul>                    <button id='pvswitcher' onclick='switchPV()'>PV Estimate</button><button onclick='evest()' id='ev_est'>選挙予想</button>";
        } else if (e.primary && !doPrimaryMode) {
            var c = "<h3>予想マップ</h3>                    <ul id='switchingEst'>" + _ + "</ul>                    <button id='pvswitcher' onclick='switchPV()'>PV Estimate</button><button onclick='evest()' id='ev_est'>Current Delegate Count</button>";
        } else {
                        ...
                        if (e.final_state_results[s].result_time > t) {
                            ...
                            for (l = 0; l < Math.min(e.final_state_results[s].result.length, 4); l++)
                                if (e.final_state_results[s].result[l].votes > 0) {
                                    ...
                                    _ += '                                <li><span style="color:' + e.candidate_json[r].fields.color_hex + "; background-color: " + e.candidate_json[r].fields.color_hex + '">--</span> ' + e.candidate_json[r].fields.first_name + ":  " + (100 * e.final_state_results[s].result[l].percent).toFixed(1) + "%</li>"
                                }
                            if (e.primary) {
    ...
    function h() {
        for (var t = f(500), i = u(), a = "", s = 0; s < i.length; s++) {
            ...
            a += '            <li><span style="color:' + i[s].color + "; background-color: " + i[s].color + '">--</span> ' + i[s].first_name + ":  " + l + "</li>"
        }
    ...
    function g() {
        ...
        var m = "";
        for (s = 0; s < l.length; s++) l[s].values.length > 0 && (m += '<option value="' + (10 + s) + '">Closest ' + l[s].first_name + " Wins</option>");
        var g = "";
        for (s = 0; s < o.length; s++) g += '<option value="' + (20 + s) + '">Highest ' + o[s].first_name + " %</option>";
    ...
    function b() {
        ...
        for (i = 0; i < e.final_overall_results.length; i++) {
            ...
            a += '            <tr><td style="text-align: left;">            <span style="background-color: ' + n + "; color: " + n + ';">----</span> ' + (e.candidate_json[s].fields.first_name) + "</td><td> " + e.final_overall_results[i].electoral_votes + " </td><td> " + M(e.final_overall_results[i].popular_votes) + " </td><td> " + (e.final_overall_results[i].popular_votes / t * 100).toFixed(e.finalPercentDigits) + "% </td></tr>"
        }
        let o;
        ...
        for (var i = "        <h4>Results - This Game</h4>        <table>\t    <tr><th>Candidate</th><th>Popular Votes</th>\t    <th>Popular Vote %</th><th>Electoral Votes</th></tr>", a = 0; a < e.final_state_results.length; a++)
            if (e.final_state_results[a].state == t)
                for (var s = 0; s < e.final_state_results[a].result.length; s++) {
                    ...
                    i += "                    <tr><td>" + (e.candidate_json[n].fields.first_name) + "</td><td>" + M(e.final_state_results[a].result[s].votes) + "</td><td>" + (100 * e.final_state_results[a].result[s].percent).toFixed(e.statePercentDigits) + "</td><td>" + e.final_state_results[a].result[s].electoral_votes + "</td></tr>"
                }
        return i += "</table>"
    }
            ...
            function (t, a) {
                ...
                for (var n = "", l = 0; l < e.running_mate_json.length; l++)
                    if (e.running_mate_json[l].fields.candidate == a) {
                        ...
                        n += "<option value=" + e.candidate_json[_].pk + ">" + e.candidate_json[_].fields.first_name + "</option>"
                    }
    ...
    candSel = (a) => {
        a.preventDefault(),
            function (a) {
                ...
                for (var n = "", l = 0; l < e.candidate_json.length; l++) e.candidate_json[l].fields.election == a && 1 == e.candidate_json[l].fields.is_active && (n += "<option value=" + e.candidate_json[l].pk + ">" + e.candidate_json[l].fields.first_name + "</option>");
